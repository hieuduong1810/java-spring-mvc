...
public class ProductSpecs {
    ...
    public static Specification<Product> matchPrice(double min, double max) {
        return (root, query, criteriaBuilder) -> criteriaBuilder.and( // criteriaBuilder.and(...): Kết hợp hai điều kiện
                                                                      // trên bằng phép AND logic.
                criteriaBuilder.gt(root.get(Product_.PRICE), min), // Lớn hơn min
                criteriaBuilder.le(root.get(Product_.PRICE), max) // Nhỏ hơn hoặc bằng max
        );
    }
